...
module.exports = function createTemplate(formName, options, callback) {
  ...
  if(options["lambdaFunction"]){
    ...
    options["lambdaFunction"] = fs.readFileSync(`./forms/${formName}/lambdaFunction.js`, 'utf8')
  }
  optionArgs(formName, args, options,function(err, data){
    if(err) console.error(err.message)
      ...
      generateTemplate(formName, formModel, requiredFields, options.email, callback)
    }
  })
  
 
  function generateTemplate(formName, formModel, requiredFields, email, callback){
    var template = {
