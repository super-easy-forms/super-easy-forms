...
module.exports = function createTemplate(formName, options, callback) {
  ...
  optionArgs(formName, args, options, function(err, data){
    if(err) optionError(err, callback)
      ...
      args = data;
      if(options && options["lambdaFunction"]){
        args["lambdaFunction"] = options["lambdaFunction"]
        fs.writeFileSync(`./forms/${formName}/lambdaFunction.js`, args["lambdaFunction"])
      }
      else{
        let obj = fs.readFileSync(`./forms/${formName}/lambdaFunction.js`, 'utf8');
        if(obj) {
          args["lambdaFunction"] = obj;
        }
        else optionError('A valid lambda function hasnt been provided.', callback)
      }
      ...
      generateTemplate(formName, args["lambdaFunction"], formModel, requiredFields, args.email, callback)
    }
  })
