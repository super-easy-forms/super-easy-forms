...
var fs = require("fs");
var {optionArgs, optionCallback} = require('./InternalModules')
  ...
  optionArgs(formName, args, options,function(err, data){
    if(err) console.error(err.message)
      ...
      if(options["lambdaFunction"]){
        args["lambdaFunction"] = options["lambdaFunction"]
      }
      else{
        let obj = fs.readFileSync(`./forms/${formName}/lambdaFunction.js`, 'utf8');
        if(options["lambdaFunction"]){
          fs.writeFile(`./forms/${formName}/lambdaFunction.js`, options["lambdaFunction"], function(err, data){
            if(err){
              throw new Error(err)
            }
            else{
          ...
          options["lambdaFunction"] = obj
        }
        else throw new Error('A valid lambda function hasnt been provided.')
      }
      ...
      generateTemplate(formName, options["lambdaFunction"], formModel, requiredFields, options.email, callback)
    }
  })
  
 
  function generateTemplate(formName, lambdaFunction, formModel, requiredFields, email, callback){
    var template = {
