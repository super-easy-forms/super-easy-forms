...
module.exports = function CreateForm(formName, options, callback) {
	...
	if(options["endPointUrl"]) FormConfig.AddVar(formName, "endPointUrl", options["endPointUrl"]);
	if(options["formFields"]) FormConfig.AddVar(formName, "fields", options["formFields"]);
	FormConfig.CheckForm(formName, function(err, data){
		if(err){
			...
			if(data){
				...
				if(!options["endPointUrl"]) options["endPointUrl"] = obj.endPointUrl;	
				if(!options["formFields"]) options["formFields"] = obj.fields; 
			}
			//write the form
			//
			//
