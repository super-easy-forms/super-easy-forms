...
function createLambdaFunction(formName, options, callback) {
	...
	let args = {"email": "", "formFields":"", "recipients":[], "captcha":false, "zip":false, "functionBucket":false}
	...
	optionArgs(formName, args, options, function(err, data){
		if(err) optionError(err, callback)
			...
			if(!options['emailMessage']){
				options['emailMessage'] = '<h4>New Contact</h4><br><p>Someone has just filled out your super easy form! bellow are their details: <br> <FormOutput>'
			}
...
function copyModules(destination, callback){
	...
	let npmPacks = ['uuid', 'axios', 'follow-redirects']
	for(let i=0; i<=npmPacks.length; i++){
		if(!npmPacks[i]){
			...
			let pack = npmPacks[i];
			let origin = `./node_modules/super-easy-forms/node_modules/${pack}`
